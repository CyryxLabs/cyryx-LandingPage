...
export function HeaderDropdown({ group, isActive }: HeaderDropdownProps) {
    ...
    <NavigationMenuItem>
      <NavigationMenuTrigger
        className={cn(
          ...
          isActive
            ? "text-[var(--silver)] after:absolute after:left-0 after:right-0 after:-bottom-0.5 after:h-px after:bg-[var(--accent-glow)] after:shadow-[0_0_6px_var(--accent-glow)]"
            : "text-[var(--silver-dim)] hover:text-[var(--silver)]"
        )}
      >
        {group.label}
        <ChevronDown 
          className="ml-0.5 h-3 w-3 text-[var(--silver-dim)] transition-transform duration-200 group-data-[state=open]:rotate-180" 
          aria-hidden="true" 
        />
      </NavigationMenuTrigger>
