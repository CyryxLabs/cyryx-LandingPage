...
import {
  ...
  MaaxWaitlistSchema,
} from "@/lib/maax-waitlist.schema";
import { trackCta } from "@/lib/track-cta";

type FieldErrors = Partial<Record<"fullName" | "email" | "phone" | "country" | "consent", string>>;
    ...
    return (
      ...
      >
        ...
        <p className="mt-4 max-w-lg text-[15px] leading-relaxed text-[var(--silver-dim)]">
          MAAX Studio access will open in waves. We will contact you when your profile aligns with
          an upcoming early-access group.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} onFocus={trackStart} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Telephone" id="maax-phone" error={errors.phone}>
          <input
            id="maax-phone"
            name="phone"
            type="tel"
        ...
        <Field label="Country" id="maax-country" error={errors.country}>
          <input
            id="maax-country"
            name="country"
            type="text"
      ...
      <label className="mt-6 flex cursor-pointer items-start gap-3 text-sm leading-relaxed text-[var(--silver-dim)]">
        ...
        />
        <span>
          I agree that Cyryx Labs may use these details to contact me by email or telephone about
          MAAX Studio early access and product updates. This does not include marketing text
          messages. I can withdraw consent at any time. See the{" "}
          <Link
            to="/privacy"
      ...
      <p className="mt-4 font-mono text-[9px] uppercase tracking-[0.16em] text-[var(--steel)]">
        No public launch date · Access opens in reviewed waves
      </p>
    </form>
  );
}
...
function Field({
  ...
  return (
    <label htmlFor={id} className="block font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--silver-dim)]">
      {label}
      {children}
      {error ? (
        <span
