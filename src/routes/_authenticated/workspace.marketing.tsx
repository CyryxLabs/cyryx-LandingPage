...
import {
  ...
  WorkspaceCard,
  WsButton,
} from "@/components/cyryx/workspace/WorkspaceShell";
        ...
        <DataTable
          ...
          fields={[
            { key: "name", label: "Name", type: "text", required: true },
            {
              ...
              label: "Kind",
              type: "select",
              options: ["paid", "organic", "referral", "outbound", "event", "partner"],
            },
      ...
      {tab === "waitlist-archive" && (
        <DataTable
          ...
          fields={[
            ...
            { key: "company", label: "Company", type: "text", className: "min-w-[180px]" },
            { key: "role", label: "Role", type: "text", className: "min-w-[160px]" },
            {
              key: "use_case",
              label: "Use case",
              type: "text",
...
function AttributionTable() {
  ...
  const ownerById = useMemo(() => {
    ...
    (campaignsMeta as any[]).forEach((c) => {
      if (c.owner_id) m.set(c.id, c.owner_id);
    });
    return m;
  }, [campaignsMeta]);
  ...
  const filterOptions = useMemo(() => {
    ...
    return {
      ...
      owners: Array.from(
        new Set((campaignsMeta as any[]).map((c) => c.owner_id).filter(Boolean)),
      ) as string[],
  ...
  const { data: auditRes } = useQuery({
    ...
    queryFn: async () => {
      let query = (supabase as any)
        ...
        .select(
          "id,ran_at,ran_by,range_key,scanned,marked_won,cleared,pipeline_before,pipeline_after,revenue_before,revenue_after,affected_lead_ids,affected_deal_ids",
          { count: "exact" },
        )
  ...
  async function exportAuditPDF() {
    ...
    if (!win) return;
    const totalPipelineDelta = rows.reduce(
      (a, r) => a + (Number(r.pipeline_after) - Number(r.pipeline_before)),
      0,
    );
    const totalRevenueDelta = rows.reduce(
      (a, r) => a + (Number(r.revenue_after) - Number(r.revenue_before)),
      0,
    );
      ...
      <WorkspaceCard>
        <div className="flex items-center justify-between px-3 py-2 border-b border-[color-mix(in_oklab,var(--accent-glow)_10%,transparent)]">
          ...
          <div className="flex gap-2">
            <WsButton
              onClick={() =>
                downloadCSV(`attribution-${new Date().toISOString().slice(0, 10)}.csv`, filtered)
              }
            ...
            <WsButton
              onClick={() => {
                if (lastResult)
                  downloadCSV(
                    `reconcile-diff-${lastResult.ran_at.slice(0, 10)}.csv`,
                    lastResult.diff,
                  );
              }}
              disabled={!lastResult}
        ...
        <div className="flex flex-wrap gap-2 px-3 py-2 border-b border-[color-mix(in_oklab,var(--accent-glow)_10%,transparent)]">
          <FilterSelect
            ...
            options={filterOptions.channels.map((v) => ({ value: v, label: v }))}
          />
          <FilterSelect
            label="Campaign"
            value={cpFilter}
          ...
          <FilterSelect
            ...
            options={filterOptions.owners.map((v) => ({ value: v, label: v.slice(0, 8) }))}
          />
          <FilterSelect
            label="Stage"
            value={stFilter}
            onChange={setStFilter}
            options={filterOptions.statuses.map((v) => ({ value: v, label: v }))}
          />
            ...
            <tbody>
              {isLoading && (
                <tr>
                  <td
                    colSpan={11}
                    className="px-3 py-6 text-center text-xs text-[var(--silver-dim)]"
                  >
                    Loading…
                  </td>
                </tr>
              ...
              {!isLoading && filtered.length === 0 && (
                <tr>
                  <td
                    colSpan={11}
                    className="px-3 py-8 text-center text-xs text-[var(--silver-dim)]"
                  >
                    No campaigns yet.
                  </td>
                </tr>
              ...
              {filtered.map((r: any) => {
                const roi =
                  r.spend > 0
                    ? ((Number(r.won_value) - Number(r.spend)) / Number(r.spend)) * 100
                    : null;
                return (
                  <tr
                    key={r.campaign_id}
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right text-[var(--accent-glow)]">
                      {fmt(Number(r.won_value))}
                    </td>
    ...
    <WorkspaceCard>
      <div className="px-3 py-2 border-b border-[color-mix(in_oklab,var(--accent-glow)_10%,transparent)]">
        <p className="hud-label text-[10px] text-[var(--silver-dim)]">
          Reconciliation diff · {new Date(result.ran_at).toLocaleString()} · range{" "}
          {result.range_key ?? "—"}
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 text-sm">
        ...
        <Kpi
          label="Pipeline after"
          value={fmtMoney(result.pipeline_after)}
          tone={pDelta === 0 ? undefined : pDelta > 0 ? "up" : "down"}
        />
        <Kpi label="Revenue before" value={fmtMoney(result.revenue_before)} />
        <Kpi
          label="Revenue after"
          value={fmtMoney(result.revenue_after)}
          tone={rDelta === 0 ? undefined : rDelta > 0 ? "up" : "down"}
        />
      </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {result.diff.map((d) => (
                <tr
                  key={d.lead_id}
                  className="border-t border-[color-mix(in_oklab,var(--accent-glow)_8%,transparent)]"
                >
                  <td
                    className={`px-3 py-1.5 ${d.action === "marked_won" ? "text-[var(--accent-glow)]" : "text-[color-mix(in_oklab,var(--silver)_60%,transparent)]"}`}
                  >
                    {d.action.replace("_", " ")}
                  </td>
                  ...
                  <td className="px-3 py-1.5 text-[var(--silver-dim)]">
                    {d.before_converted_at?.slice(0, 10) ?? "—"}
                  </td>
                  <td className="px-3 py-1.5 text-[var(--silver-dim)]">
                    {d.after_converted_at?.slice(0, 10) ?? "—"}
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <WorkspaceCard>
      <div className="px-3 py-2 border-b border-[color-mix(in_oklab,var(--accent-glow)_10%,transparent)] flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center">
          ...
          />
          <WsButton onClick={() => onPage(page - 1)} disabled={!hasPrev}>
            Prev
          </WsButton>
          <WsButton onClick={() => onPage(page + 1)} disabled={!hasNext}>
            Next
          </WsButton>
          <WsButton
            onClick={() => downloadCSV(`attribution-audit-p${page + 1}.csv`, rows)}
            disabled={!rows.length}
          >
            Export page
          </WsButton>
          <WsButton onClick={() => onExportAllCSV()} disabled={total === 0}>
            Export CSV (all)
          </WsButton>
          <WsButton onClick={() => onExportPDF()} disabled={total === 0}>
            Export PDF
          </WsButton>
        </div>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td colSpan={8} className="px-3 py-6 text-center text-[var(--silver-dim)]">
            ...
            {rows.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-1.5 text-right text-[var(--accent-glow)]">
                    {r.marked_won}
                  </td>
                  <td className="px-3 py-1.5 text-right">{r.cleared}</td>
                  <td
                    className={`px-3 py-1.5 text-right ${pd > 0 ? "text-[var(--accent-glow)]" : pd < 0 ? "text-[color-mix(in_oklab,var(--silver)_50%,transparent)]" : ""}`}
                  >
                    {fmtMoney(pd)}
                  </td>
                  <td
                    className={`px-3 py-1.5 text-right ${rd > 0 ? "text-[var(--accent-glow)]" : rd < 0 ? "text-[color-mix(in_oklab,var(--silver)_50%,transparent)]" : ""}`}
                  >
                    {fmtMoney(rd)}
                  </td>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone?: "up" | "down" }) {
  const color =
    tone === "up"
      ...
      : tone === "down"
        ? "text-[color-mix(in_oklab,var(--silver)_55%,transparent)]"
        : "";
  return (
    <div>
  ...
  return (
    <div className="hidden print:block print:!bg-white print:!text-black">
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-4 gap-3 text-sm">
          ...
          <PrintKpi
            ...
            value={
              totals.spend > 0
                ? `${(((totals.won_value - totals.spend) / totals.spend) * 100).toFixed(0)}%`
                : "—"
            }
          />
          {result && (
            <PrintKpi
              label="Pipeline Δ"
              value={fmtMoney(result.pipeline_after - result.pipeline_before)}
            />
          )}
          {result && (
            <PrintKpi
              label="Revenue Δ"
              value={fmtMoney(result.revenue_after - result.revenue_before)}
            />
          )}
        </div>
      </section>
      ...
      {result &&
        ...
        (() => {
          ...
          for (const d of result.diff) {
            ...
            if (d.action === "marked_won") {
              cur.marked_won++;
              cur.deal_value_delta += d.deal_value;
            } else {
              cur.cleared++;
              cur.deal_value_delta -= d.deal_value;
            }
          ...
          const rows = Array.from(byCampaign.entries()).map(([cid, v]) => ({
            campaign_id: cid,
            name: nameById.get(cid) ?? cid.slice(0, 8),
            ...v,
          }));
          return (
      ...
      <p className="mt-6 text-[9px] text-gray-500">
        Range totals by stage aggregated from mkt_attribution_v for the selected range · Cyryx Labs
      </p>
    </div>
  );
}
