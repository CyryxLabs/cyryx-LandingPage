...
export interface Publication {
  ...
  affiliation?: string;
  date: string;
  publishedAt: string; // ISO for "new" detection
  version?: string;
  documentId?: string;
  license?: string;
  licenseUrl?: string;
...
This document introduces the Cyryx Governance Protocol (CGP) v1.0, a technical framework that fills these gaps with seven control domains and twenty-eight normative controls (MUST/SHOULD/MAY). CGP is designed as an extension to existing frameworks — not a replacement — with explicit mapping to EU AI Act Articles 9, 12, 13, 14, and 15; NIST AI RMF functions GOVERN, MAP, MEASURE, and MANAGE; and ISO 42001 Clause 6, 7, 8, and 9 controls. Every control in CGP v1.0 has a reference implementation in MAAX Studio by Cyryx Labs. CGP is published under Creative Commons Attribution 4.0 (CC BY 4.0) for open community adoption and review.`,
  ...
  frameworkMapping: [
    { domain: "CD1 — Mission Authorization", euAiAct: "Art. 14, Art. 9", nist: "GOVERN 1.1, MAP 1.5", iso: "Clause 6.1, Annex A-6.2" },
    { domain: "CD2 — Scope Boundary", euAiAct: "Art. 9, Art. 13", nist: "MAP 3.5, MEASURE 2.5", iso: "Annex A-8.4, Clause 8.1" },
    { domain: "CD3 — Checkpoint Integrity", euAiAct: "Art. 9.4", nist: "MANAGE 2.2, MANAGE 3.1", iso: "Annex A-8.5, Annex A-9.1" },
    { domain: "CD4 — Command Gates", euAiAct: "Art. 9, Art. 15", nist: "MEASURE 2.1, MEASURE 2.3", iso: "Annex A-8.3, Annex A-8.6" },
    { domain: "CD5 — Decision Attribution", euAiAct: "Art. 12, Art. 14.4", nist: "GOVERN 6.1, GOVERN 6.2", iso: "Annex A-8.2, Clause 7.5" },
    { domain: "CD6 — Evidence Chain", euAiAct: "Art. 12, Art. 17", nist: "GOVERN 6.2, MEASURE 2.8", iso: "Clause 9.1, Annex A-8.7" },
    { domain: "CD7 — Cost Governance", euAiAct: "Not explicitly addressed", nist: "GOVERN 4.1, MANAGE 4.1", iso: "Annex A-8.4, Clause 9.1" },
  ],
  conformanceLevels: [
    { level: "CGP-L1", name: "Agentic Baseline", requirement: "All MUST controls in CD1, CD3, CD5, CD6", badge: "BASELINE", accent: "muted" },
    { level: "CGP-L2", name: "Governed Execution", requirement: "All MUST controls across all 7 domains", badge: "GOVERNED", accent: "emerald" },
    { level: "CGP-L3", name: "Enterprise Agentic Governance", requirement: "All MUST and SHOULD controls + third-party review", badge: "ENTERPRISE", accent: "white" },
  ],
...
export function hasNewPublication(withinDays = 30): boolean {
  ...
  return PUBLICATIONS.some(
    (p) => p.status === "new" || new Date(p.publishedAt).getTime() >= cutoff,
  );
}